...
@section('content')
    <div class="container">
        <div class="row">
           <div class="col-10">
            <form method="POST" action="{{ route('triplocation.update',$tripLocation->id) }}">
                @csrf
	      @method('PUT')
                <div class="mb-3">
                    ...
                    <select class="form-control" name="trip_from" id="trip_from">
                        ...
                        {{--@foreach ($$trips as $trip)
                            <option value="{{$trip->id}}" {{ $trip->id == $tripLocation->trip_from ? 'selected' : ''}}>{{$trip->name}}</option>
                        @endforeach--}}
		    @foreach ($trips as $trip)
		    <option value="{{ $trip->id }}" {{ optional($tripLocation)->trip_from == $trip->id ? 'selected' : '' }}>
		        {{ $trip->name }}
		    </option>
		@endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="trip_location_name" class="form-control" id="trip_location_name"
                        value="{{$tripLocation->trip_location_name ?? "" }}">
        ...
        function updateTripToOptions() {
            ...
            @foreach ($trips as $trip)
                if ("{{ $trip->id }}" !== selectedTripFrom) {
                    $('#trip_to').append('<option value="{{ $trip->id }}" {{ $trip->id == $tripLocation->trip_to ? 'selected' : ''}}>{{ $trip->name }}</option>');
                }
            @endforeach
        }
    });
